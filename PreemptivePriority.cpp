...
// Assuming lower number has higher priority
#include <iostream>
#include <vector>
#include <algorithm>
...
void display() {
    ...
    float totalTAT = 0;
    float avgWT = 0;
    cout << endl << "Process\tArrival Time\tBurst Time\tPriority\tWaiting Time\tTurn Around Time\tCompletion Time" << endl;

    for(Process p : processList) {
        cout << "P" << p.name << "\t\t" << p.arrivalTime << "\t\t" << p.burstTime << "\t\t" << p.priority << "\t\t" << p.waitingTime << "\t\t" << p.turnAroundTime << "\t\t" << p.completionTime<< endl;
